...
#Sets the device to use, CUDA if NVIDIA CUDA drivers are available, cpu otherwise
device = None
if device is None:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

#Loads pre-trained model from save and set the device and puts it into eval mode instead of train mode
